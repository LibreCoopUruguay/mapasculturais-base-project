...
- **Características**:
    ...
    - **Multi-Factor Authentication (MFA)**:
        ...
        - Sistema basado en tokens temporales seguros
        - UI profesional con badge de estado visual (Activado/Desactivado)
        - Email template responsive con diseño profesional
        - Endpoints: `POST /auth/toggle_mfa`, `GET /auth/get_mfa_status`, `POST /auth/verify_mfa`, `POST /auth/resend_mfa`
    ...
    - **Vistas**: `views/auth/` (registro, login, recuperar contraseña, MFA).
    - **Componentes**: `login`, `mfa-verify`, `change-password` (con toggle MFA).
    - **Clase Principal**: `Provider.php`. Controla el flujo `auth.register`, `auth.login`, `auth.recover`, `auth.mfa`.
